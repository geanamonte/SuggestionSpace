...
import dwws.suggestionspace.application.InstallSystemService;
...
public class InstallSystemController extends JSFController {
	...
	/** Input: the useristrator being registered during the installation. */
	private User user = new User();

	/** Input: the repeated password for the userinstrator registration. */
	...
	/**
	 * Analyzes the name that was given to the useristrator and, if the short name field is still empty, suggests a
	...
	private boolean checkPasswords() {
		if (((repeatPassword != null) && (!repeatPassword.equals(user.getPwd()))) || ((repeatPassword == null) && (user.getPwd() != null))) {
	...
	public String saveConfig() {
		...
		// Installs the system.
		//try {
			installSystemService.installSystem(config, user);
		//}
